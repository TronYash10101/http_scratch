...
void fill_headers(request_headers *headers, const char *field_name_buffer,
    ...
    if (strcasecmp(field_name_buffer, "Host") == 0) {
        snprintf(headers->Host, sizeof(headers->Host), "%s",
                 field_value_buffer);
        ...
        snprintf(headers->Accept_Encoding, sizeof(headers->Accept_Encoding),
                 "%s", field_value_buffer);
    } else if (strcasecmp(field_name_buffer, "Accept-Language") == 0) {
        snprintf(headers->Accept_Language, sizeof(headers->Accept_Language),
                 "%s", field_value_buffer);
...
void request_headers_parser(const char *restrict header,
    ...
    while (1) {
        ...
        int name_word_count;
        for (name_word_count = 0; header[word_count] != ':';
             name_word_count++) {
        ...
        if (recording_value == true) {
            ...
            for (value_word_count = 0; !(header[word_count] == '\r' &&
                                         header[word_count + 1] == '\n');
                 value_word_count++) {
...
void request_parser(const char *restrict req_get, Irequest_line *request_line,
    ...
    if (headers->Content_Length > 0) {
        word_count++;
        for (body_word_count = 0; req_get[word_count] != '\0';
             body_word_count++) {
